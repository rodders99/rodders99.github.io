<?php

echo '
<nav>
<ul>
    <li><a href="#">Site Navigation</a>
        <ul>
           <li><a href = "/projects/fsa-safety-alerts/fsa-safety-alerts/index.html">Project Home</a></li>
        </ul>

       <li>
        <a href = "#">Analyst&#39;s Notebooks</a>
           <ul>
               <li><a href = "./a0-fsa-meta-data.php">Meta Data Overview</a></li>
               <li><a href = "./a1-fsa-feature-engineering.php">Feature Engineering</a></li>
               <li><a href = "./a2-fsa-front-end.php">Web Scrape Front End</a></li>
           </ul>
       </li>
       <li>
       <a href = "#"><b>Report</b></a>
          <ul>

          <li><a href="./00-fsa-analysis-introduction.php">FSA - Analysis Introduction</a></li>
          <li><a href="./01-fsa-summary.php">FSA - Analysis Summary</a></li>
          <li><a href="./02-fsa-pathogens.php">FSA - Pathological Contaminants Analysis</a></li>
          <li><a href="./03-fsa-allergens.php">FSA - Allergen Contaminants Analysis</a></li>
          <li><a href="./04-fsa-foreign-material.php">FSA - Foreign Material Contaminants Analysis</a></li>
          <li><a href="./05-fsa-other.php">FSA - Other Contaminants Analysis</a></li>
          <li><a href="./06-fsa-product-type.php">FSA - Product Type</a></li>

          
              
          </ul>
      </li>
       
    <!--   <li><a href="#"<h2>EV Headlines</h2></a></li> -->
      
 </ul>
</nav>


'; 